 ...
 *
 * pg_weave_aux.c
 *		Auxiliary functions for pg_weave: highlight, snippet, score explain.
 *
 * Stage 8 of pg_weave.  These operate on the original text plus a query and are
...
static bool
query_has_term(WeaveQuery q, const char *folded, int len)
{
	uint32		i;
	...
	for (i = 0; i < q->nitems; i++)
	{
		...
		if (it->type == WEAVE_QI_VAL &&
			(int) it->termlen == len &&
			memcmp(WEAVE_QUERY_ITEMTEXT(q, it), folded, len) == 0)
			return true;
	}
	return false;
}
...
static void
tokenize_and_mark(const char *src, int len, WeaveQuery q,
...
PG_FUNCTION_INFO_V1(weave_highlight);

/* weave_highlight(text, wquery, pre text, post text) -> text */
Datum
weave_highlight(PG_FUNCTION_ARGS)
{
...
PG_FUNCTION_INFO_V1(weave_snippet);

/*
 * weave_snippet(text, wquery, pre, post, ellipsis, max_tokens) -> text
 ...
 */
Datum
weave_snippet(PG_FUNCTION_ARGS)
{
	text	   *doc = PG_GETARG_TEXT_PP(0);
	WeaveQuery	q = PG_GETARG_WQUERY(1);
