 ...
 * weave AM uses at query time is in pg_weave_trgm_index.c.
 *
...
/*
 * weave_regex_trigrams -- extract required trigrams from a regular expression by
 ...
 */
int
weave_regex_trigrams(const char *re, int relen, uint32 *out, int maxout)
{
	int			n = 0;
	int			i = 0;
	char		run[256];
	...
	do {															\
		...
		if (_rl >= 3)												\
		{															\
			int _t = weave_trigrams(run, _rl, out + n, maxout - n);	\
			n += _t;												\
		}															\
		runlen = 0;													\
	} while (0)
