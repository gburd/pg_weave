 ...
 */
#define WEAVE_LEV_INF (aut->k + 1)

static inline void
weave_lev_start(const WeaveLevAut *aut, int16 *row)
{
	int			j;
...
static inline int
weave_lev_step(const WeaveLevAut *aut, const int16 *prev, unsigned char c, int16 *next)
{
	int			j;
	int			rowmin;
...
static inline bool
weave_lev_accept(const WeaveLevAut *aut, const int16 *row)
{
 ...
 */
/*
 * Like weave_lev_match_prefix below but the simple form; retained as the
 * correctness reference is weave_lev_match_prefix, which also reports the dead
 * prefix.  (Plain weave_lev_match removed -- all callers use the prefix form.)
 */
...
{
	int16		rowa[WEAVE_LEV_MAXQ + 2];
	int16		rowb[WEAVE_LEV_MAXQ + 2];
	int16	   *cur = rowa;
	int16	   *nxt = rowb;
	int			i;

	*deadlen = candlen;
	weave_lev_start(aut, cur);
	for (i = 0; i < candlen; i++)
	{
		int			rmin = weave_lev_step(aut, cur, cand[i], nxt);
		int16	   *t = cur;
	...
	return weave_lev_accept(aut, cur);
}
