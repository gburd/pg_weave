...
-- must survive the paths that MOVE pages.
--
-- The design rests on this and nothing else: `codestart + b * strips_per_block`
-- was measured at 94 % correct on a freshly merged weft and **5 %** after a
-- DELETE + VACUUM rewrite, because a rewrite draws recycled pages from the FSM
-- (bench/RESULTS_GATE_SWEEP.md).  So the stored pointer is what makes single-block
-- access sound, and a rewrite is exactly where a writer that forgot to maintain it
-- would still pass every other test -- the answers stay plausible, they are just
-- computed from another block's codes, which is why weave_check() has an invariant
...
-- COVERAGE, not the pointer itself, and the difference is worth stating because the
-- obvious reading of this query is wrong: `firstpage` has no SQL surface, so nothing
-- here compares it to anything.  What this asserts is that the two readers agree on
-- which blocks EXIST -- weave_vec_strips() walks the code chain and reports each
-- page's own header, weave_vec_blocks() reads through the directory -- so a block
-- that one can see and the other cannot is caught.  The pointer comparison is
-- weave_check()'s vector_block_stats_match_codes invariant above, which reads the
-- record and then reads the page it names; that is the assertion with teeth, and the
-- mutation that stores firstpage + 2 makes it fire.
SELECT count(*) AS blocks_the_two_readers_disagree_about
  FROM (SELECT blockno FROM weave_vec_strips('vwg27_weave') GROUP BY blockno) s
  FULL JOIN (SELECT blockno FROM weave_vec_blocks('vwg27_weave')) b USING (blockno)
 WHERE s.blockno IS NULL OR b.blockno IS NULL;
