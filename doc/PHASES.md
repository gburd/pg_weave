...
| **F6** | **NEW 2026-09-21, created by F2 reading the code: the BM25 lexical channel has no shuttle, so the flagship fused query cannot be executed.** Ranked lexical is a Broder/BMW WAND over `WandCursor` (`src/am/amscan.c:4361-4423`, pivot loop `:5024-5157`), and the fused core consumes only `WeaveShuttle`s. **Scope is glue, not a format change, and that was checked rather than assumed:** `WandCursor` already carries the current docid, the decoded block (so `blkend` is `docids[blkcount-1]`), the per-block bound inputs `blk_max_tf`/`blk_min_dl` with `wand_block_max_contrib()` (`:4605-4616`) computing the bound, and the term-wide ceiling `max_contrib`; `WeaveBlockHdr` (`include/weave/am.h:350-363`) already stores `max_tf` and `min_doclen` per block. **No new on-disk field.** One shuttle per query TERM, which is what `channel.h:170` and `fuse.h`'s note already assume — so a multi-term BM25 query becomes several scored channels under one threshold, and the fused core subsumes the WAND rather than sitting on top of it. | (C1)+(C2) property test per hard rule 1: `bound ≥ score` over random postings against the existing WAND as oracle, plus equality of the top-k with `weave_topk_visible()` on the same query |
| **F7** | **NEW 2026-09-21, created by F2 reading the code: the vector channel has no `ORDER BY` operator, so its query cannot reach `amrescan` at all.** `wvec_weave_ops` (`sql/pg_weave--0.7.0--0.8.0.sql:99`) is `STORAGE wvec` and nothing more; V8's scan is reachable only through the `weave_vec_scan()` SRF, and the `<=>`/`<->`/`<#>`/`<+>` operators on `wvec` (`sql/pg_weave--0.1.0--0.2.0.sql:119-134`) are ordinary functions outside any opfamily — which is why the fused **fallback** works today and the pushdown cannot. Owes: the opfamily ORDER BY member, the `amrescan` key path, and a single-channel ordered vector scan over the existing `weave_vec_shuttle_begin()`. **This is Phase V's debt surfacing in Phase F**; it is recorded here rather than added to V because F2 is what needs it and V's own gate does not. | `ORDER BY wvec_col <=> $1 LIMIT k` plans an index scan and returns the same rows as `weave_vec_scan()` for the same k, on both majors |
