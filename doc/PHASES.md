...
B/vector); (3) **MEASURED 2026-09-13 AND MISSED BY ~40×**.

`bench/RESULTS_CODE_SCAN.md`: a flat scan of 1M 960-d 4-bit codes is **291 ms**
through the fastest kernel this project has, against a bar of ~7.1 ms (2× pgvector
HNSW's warm 3.548 ms), before the rerank is added. Two measurements make that a
structural miss rather than a tuning gap:

- **The block bound prunes 0.00%** on GIST-960d and 0.01% on GloVe-200d, at an
  oracle threshold, with one k-means cluster per block. `FUSED_TOPK` §2's vector
  bound is sound and useless: for L2-normalized data the Cauchy–Schwarz term is
  ≈ 1.0 by construction while θ is always under 1.0, and the residual term is loose
  by a factor growing with √dim. So there is no pruning to be had.
- **The scan is compute-bound, `O(dim × nvec)`**, proven by per-vector cost being
  flat to 0.5% from n = 50k (23 MB of codes, L3-resident) to n = 1M (458 MB, DRAM).
  Bandwidth-bound work would differ between those points.

Only two things can move it: **scan fewer vectors** (candidate reduction) or
**score fewer coordinates** (dimensionality reduction, or a two-stage scan over a
coordinate prefix — not currently in this ledger). Ruled out by measurement:
narrower codes, a different SIMD kernel, and warp ordering.

**This is the phase's blocking problem, and it needs a maintainer decision** —
see the V9 note below.
