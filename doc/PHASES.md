...
| ~~**V7**~~ | **DONE 2026-09-16, and the format is now v8.** The on-disk shape that blocked it is **(B) coordinate-sliced pages** -- a page holds one coordinate range `[j0, j1)` of one block's 32 lanes, exact under `WEAVE_PACK_LANE` (code `(j, s)` sits at bit `(j*32+s)*bits`, so a coordinate is exactly `4*bits` **bytes**) and refused under `WEAVE_PACK_VECMAJOR`. Arithmetic and the two rejected shapes: `doc/specs/VECTOR_CHANNEL.md` §7; ratified format §7.1; the AM's attribute surface §7.2; the writer's two producers §7.3. **Landed:** the strip geometry and writer/reader (`include/weave/vecweft.h`, `src/vector/vecweft.c` PG-free, `src/vector/vecwrite.c` backend, under GenericXLog); the O(1)-addressable **block directory** a dim-wide centroid code forced into existence; the **(C2) bound** `weave_vecblock_stats()`; the **AM attribute surface** (`amcanmulticol`, `wvec_weave_ops`, `weave_index_layout()`); chandesc registration (VECTOR is kind 2, so the descriptor goes **between** LEXICAL and FUZZY or the validator rejects the page); `amcheck` invariants `vector_coverage` and `vector_block_stats_match_codes` that recompute the directory from the stored codes; `size_detail` buckets; the `weave_vec_meta`/`weave_vec_blocks`/`weave_vec_strips` introspection. **The version bump to v8 is about FREEING, not parsing** -- a v7 binary reads the descriptor page fine and frees only the wefts it knows, so every merge under it leaks the whole weft; same reasoning as v7's own bump. **Deliberately not done, and stated as the blocker on V8:** merge producer 2. A merge SKIPS any vector-bearing bolt (§7.3) rather than dropping the weft (silent loss nothing can notice before V8) or throwing (an error reachable from VACUUM cleanup is how an index becomes unvacuumable -- G15, G20). Stated cost: **a vector index does not compact, so its segment count only grows.** That also makes the free path unreachable dead code today -- enumerated, measured (an `elog(ERROR)` at its head passes the whole suite) and recorded as **G24**. **Mutation record: 4 of 9 initially survived; 8 of 9 now caught and the ninth is provably uncatchable until G24 closes.** Two of the coordinator's three diagnoses of those survivors were wrong, and the fourth hole was in the mutation harness itself -- a build error reported as a caught mutation, hiding a docid-ordering bug that only a **synchronized scan starting mid-relation** can expose (`t/017_vector_syncscan.pl`, which needs its own cluster because `installcheck` cannot set `shared_buffers`). | New page kinds `WEAVE_PK_VMETA`/`WEAVE_PK_VDIR`/`WEAVE_PK_VCODES`, read with `WeavePageHasKind()`, never a bitwise AND. **On-disk determinism:** `weave_block_codebytes()` returns 0-28 slack bytes no pack function writes, so the block buffer is zeroed before packing. | **MET:** crash-recovery TAP extended to a vector index (`t/016_vector_durability.pl`); `weave_check(deep)` clean after recovery; TAP 15 files / 214 tests |
