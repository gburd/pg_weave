...
| V7 | **BLOCKED 2026-09-15 on one on-disk decision, with the arithmetic in `doc/specs/VECTOR_CHANNEL.md` §7.** A 32-lane block at the ratified 4 bits is `32*ceil(dim*bits/8)` bytes of codes plus a `dim`-wide centroid header plus 256 B of lane sidecars, so it is **16,124 B at 960-d and does not fit an 8 kB page** (2 pages at 768-d and 960-d, 4 at 1536-d), while at 256-d one-block-per-page wastes **45 %**. The spec said "32-lane code blocks, each preceded by a `WeaveVecBlockHdr`" and never said how a block relates to a page. Two shapes: (A) a byte stream over a page chain, which needs a memcpy or fragment-wise scoring on a scan already ~69 % bandwidth-bound; (B) coordinate-sliced pages, where a page holds `[j0, j1)` of a block's 32 lanes -- free for `WEAVE_PACK_LANE` since a page break becomes a coordinate break and the byte kernel already accumulates across coordinates, and it would additionally turn **V15's prefix stage into an I/O reduction** (read `m/dim` of the pages) rather than the compute-only saving that measured 1.43x. Cost of choosing wrong is a REINDEX, so it is escalated. | New page kinds `WEAVE_PK_VCODES`/`WEAVE_PK_VMETA` (ids 18/17, reserved by X1 in the extended kind space -- read them with `WeavePageHasKind()`, never a bitwise AND); codes live in the bolt under GenericXLog. **On-disk determinism:** `weave_block_codebytes()` allocates 0-28 slack bytes per block beyond the tight `4*dim*bits` requirement (see the note above it in `include/weave/quantize.h`), and no pack function ever writes them, so V7 **must zero the block buffer before encoding** or two indexes holding identical vectors get different bytes on disk. | crash-recovery TAP test extended to a vector index |
