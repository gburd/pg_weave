...
| **F8** | **NEW 2026-09-21, created by F2.2 and specified by `FUSED_TOPK.md` section 7b.** Adapt the **vector** shuttle into the fused scan's docid space. The vector shuttle publishes segment-local **lane indices**; the fused core's positions are docids. §7b carries the design: materialize the bolt's docid-ascending warp map (`WEAVE_PK_VWARP` already stores it, 8 B/lane/pass) and translate `seek`/`blkend` through it, which preserves (C1) monotonicity and (C2) because a monotone relabelling of positions does neither harm. **Until this lands the flagship `fuse(body <=> q, emb <-> v)` query is answered by the fallback, not the index**, which is the honest state and is what `sql/fuse_pushdown.sql`'s refusal arm asserts. | the flagship two-channel query plans an `Index Scan`, its rows equal the exact oracle built from `weave_search()` plus `weave_vec_scan()`, and `test_fuse_props.c` gains a synthetic sparse-to-dense adapter leg |
| **F9** | **NEW 2026-09-21, created by F2.2.** Give `<@>` (Z9 edit distance) a **document-space** shuttle. Unlike F8 this is not an adapter: `src/query/edist.c` walks the **dictionary**, so its positions are dictionary entries and there is no relabelling that turns them into docids -- the channel has to publish document positions, which means merging the postings of the terms it accepts behind the cursor. That is the same fanout the Z5/Z6 rows hit, so this task is where a bounded `<@>` either gets cheap or gets recorded as a loss. | `<@>` participates in a fused pushdown; (C1)+(C2) property test per hard rule 1; and the fanout cost measured rather than assumed (hard rule 9) |
