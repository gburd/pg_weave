...
| **L18** | **NEW 2026-09-13, P0-candidate: a single-segment index never reclaims deleted space.** Delete 90% of a 120k-row corpus, `VACUUM`, then `weave_vacuum()` twice: the index goes **2289 -> 2296 pages**, `weave_vacuum()` returns **false** both times, and `weave_index_nsegments()` is **1**. Compaction is implemented as a MERGE of segments and a merge of one segment is a no-op, while insert-time tiered merge drives every index toward exactly one segment — so **the steady state is the state that cannot reclaim**, and the growth is `weave_bulkdelete` adding tombstones nothing removes. `doc/GAPS.md` G12. Found by adding a delete-then-reclaim arm to `t/008_vacuum_reclaim.pl`, which had asserted only `v1 <= built` on a freshly built index — satisfied by doing nothing. The arm came from reviewing pg_fts `2605d00`, whose own fix for the growth half of this bug class degraded into never reclaiming until its new test caught it. Fix: rewrite a single segment when its tombstone fraction crosses a threshold — a compaction that is not a merge — and note that `weave_page_recyclable()` gates reuse on `GlobalVisCheckRemovableXid()`, so pages freed by that rewrite are not reusable in the same transaction; let the next cycle reclaim them rather than extending, and refresh the FSM before deciding or the pass is skipped forever. | the two `TODO`-marked assertions in `t/008_vacuum_reclaim.pl` pass without the TODO (prove reports an unexpected success today, so this cannot rot), AND the no-growth assertions still hold, AND `weave_check()` passes after the rewrite |
