...
| F2 | **IN PROGRESS 2026-09-21, unblocked by the widened waiver above, and SPLIT INTO TWO STAGES after reading the code §7 describes — see `FUSED_TOPK.md` §7a, which corrects three of §7's claims.** `fuse()` planner support: recognize it, push it into the index `ORDER BY`, and provide an executable fallback so the query never fails. **Plan shape decided by the maintainer 2026-09-21: (A) a hand-built `IndexPath` in `set_rel_pathlist_hook`**, whose `indexorderbys` are the individual `col <=> q` `OpExpr`s plus one **transport key** (`<~>` over `(wdoc, float4[])` and `(wvec, float4[])`) that exists only so `amrescan` can see the weights; a `CustomScan` was rejected because it would re-implement `nodeIndexscan`'s heap fetch, visibility, qual recheck and EPQ, and a *qual* transport key was rejected because `indexqualorig` is re-evaluated on an EPQ recheck and the marker would eventually execute. `fuse()` is declared as **overloads for 2..8 channel arguments with a named `weights` parameter** (PostgreSQL cannot combine `VARIADIC` with a trailing named parameter). **F2.1** = SQL surface + planner recognition + fallback + `sql/fuse_fallback.sql`; it is testable with zero fusible channels, because the interesting path is the **refusal**. **F2.2** = the AM-side fused scan. **Also owes `sql/fuse_degenerate.sql`, inherited from F1**, which only becomes meaningful once F6/F7 give it a second scored channel to be degenerate against. | `sql/fuse_fallback.sql` |
