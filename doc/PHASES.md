...
| **L19** | **NEW 2026-09-14, created by L18. Reclaim under a share lock, so autovacuum can do it.** L18 fixed `weave_vacuum()` (`AccessExclusiveLock`) and deliberately left `weave_vacuumcleanup()` alone: under `ShareUpdateExclusiveLock` a concurrent scan can exist, so `weave_page_recyclable()` must keep gating on `GlobalVisCheckRemovableXid()`, phase 2 of the vacate+pack cannot reuse the pages phase 1 just freed, and the rewrite would **extend** the relation instead of shrinking it. That is not speculative — it is exactly how pg_fts produced 35 → 52 → 69 MB across three no-op cleanups, and its skip-style fix for that then degraded into never reclaiming at all until its own test caught it. So the requirement is a rewrite that is correct under a share lock: either a two-cycle scheme (free now, pack on the next cleanup, which must be proven to converge rather than ratchet) or a bound on transient growth. **Measure the current behaviour first** — `t/008`'s plain-`VACUUM` arm records 264 → 267 → 267, i.e. flat, so today's cost of the gap is "a manual `weave_vacuum()` is required", which is a documentation problem, not corruption. Do not trade it for unbounded growth. | `doc/GAPS.md` G14 (the still-open half) | repeated plain `VACUUM` on a delete-heavy index **shrinks** it, AND never returns larger than it started across at least 5 cycles, AND `t/008`'s existing no-growth assertions still hold |
...
| **L21** | **NEW 2026-09-14, from G17. Batch the WAL for page frees.** `weave_free_page()` wraps every page in its own `GenericXLogStart`/`Finish` and is called in a loop from `weave_free_chain()`/`weave_free_segment()`, so freeing a segment emits one full-page delta per page. Upstream measured the same shape at 3.8 GB / ~489k pages: its vacuum ran **113+ minutes without finishing**. **L18 raises the priority of this** — it makes `weave_vacuum()` actually rewrite tombstone-heavy segments, so the free path now runs on workloads where it previously did nothing. **Measure before building, and the arithmetic argues against building:** `GenericXLog` is capped at `MAX_GENERIC_XLOG_PAGES` (4) buffers per record, so the ceiling is **~4×, not the order of magnitude the page count suggests**. Establish on a multi-GB index whether 4× is enough to matter before writing any code; if it is not, the honest outcome is a recorded "won't fix, here is the number" like G11. | `doc/GAPS.md` G17 | a measured free-path cost on an index of at least a few GB, and then either a ≥3× improvement or a recorded decision not to pursue it |
