...
| **L17** | **Make the doclen sidecar's docid column randomly addressable.** `bench/RESULTS_SCAN_PROFILE.md`: ~72% of a ranked mid k=10 scan is `weave_doclen_cursor_load_page`, which FOR-unpacks and prefix-sums a whole 128-entry block on every block change to use ~2.6 of them. **The sequential decode is already at its floor** (~1.9M entry decodes vs a 2M floor of one per corpus document), so caching cannot fix it: whole-page caching is same-total and was already tried and reverted (~200× amplification on scattered rare terms), lazy in-block decode saves ~7%, and resuming the header walk from the current block instead of the page start is real but only ~14% of `load_page`. The only way to beat the floor is to **skip** entries, which needs random access — and `weave_for_get()` already gives O(1) access to FOR-packed entries, but cannot be used on a *gap* column because entry i's docid is a prefix sum. **So store the docid column as absolute offsets from the block's `first_docid` instead of gaps** (same codec, different values): the column becomes monotone and addressable, locating a docid inside a block becomes a **binary search of ~7 `weave_for_get` calls instead of 128 unpacks + 128 prefix-sum stores** (~105K vs ~1.9M per query), and the resident `docid[]`/`byte[]` arrays are no longer needed — the byte is read directly at the found index. Cost: coded width ~6→~13 bits, sidecar 4,672 kB → ~9 MB (**+0.7%** of a 625 MB index). Scoped segment-format change: only the sidecar docid column, and the format is already versioned (v3 inline / v4 sidecar, `weave_meta_upcast_page`). Needs a version bump, a reader that still handles v4 gap-coded sidecars, and an extension of the FOR/wire property tests. | `bench/RESULTS_SCAN_PROFILE.md`, `doc/specs/SEGMENT_FORMAT.md` | mid k=10 p50 ≤ 8 ms (from 10.26), rare ≤ 2.2 ms (from 2.82), common no regression, buffer hits per ranked mid k=10 < 3,000 (from 11,702), index size ≤ 630 MB, and a v4 index built by 0.5.0 still reads correctly after upgrade |
