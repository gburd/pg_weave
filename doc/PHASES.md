...
| V13 | **WITHDRAWN 2026-09-23, on a measurement against the product's own corpora.** Re-justification was attempted once more — as maintainer decision (c) for §8's vector blocker — and failed by the same margin on the three BEIR datasets the nDCG row uses: **0.00 % of blocks pruned, 100.00 % of lanes scored, in BOTH orderings, and 0.00 % against an ORACLE threshold** (the ceiling over all block orderings). The clustered arm's blocks really are tighter (mean radius 1.0436 → 0.9992 on scifact) and the bound needs **2.12–3.40×** tighter, not 4 %; `lists > n/32` makes it worse because a 32-lane block then spans several clusters. `bench/RESULTS_CLUSTER_ORDER.md`. So the ordering is not built, and F8's strictly-ascending-docid contract is NOT reopened. **JUSTIFICATION GONE 2026-09-13; withdraw or re-justify.** Warp ordering by cluster existed to make the block bound prune. The bound prunes 0.00% on real corpora WITH one k-means cluster per block, and natural file order measures the same 0.00% (`bench/RESULTS_CODE_SCAN.md`), so ordering the warp buys nothing measurable. It was already reduced to this single justification when V9 was demoted. Original entry: **RE-JUSTIFICATION REQUIRED 2026-09-12** — this row says "IVF satisfies this requirement inherently", and V9 (IVF) is now demoted, so the clustering has to be provided by something. It does not become optional: the block bound prunes 99.6 % of blocks with a coherent warp and **0.0 %** with a random one, so warp coherence is load-bearing regardless of whether anything probes clusters at query time. What changes is that it needs a build-time k-means of its own rather than inheriting one — the same requirement the withdrawn proximity-graph plan had. Original entry follows. **Warp ordering by cluster.** Assign warp positions in the order the IVF build's k-means clustering produces (§8a of doc/specs/VECTOR_CHANNEL.md; IVF satisfies this requirement inherently, where the withdrawn graph plan needed it as a separate constraint), so each 32-lane code block is spatially coherent. Not an optimization: `bench/RESULTS_BOUND_PRUNING.md` measures the block bound pruning 99.6% of blocks with a coherent warp and **0.0%** with a random one. | `bench/bound_pruning.c` reports ≥ 90% blocks pruned at k=10 on the shipped corpora; a heap-order build is rejected by the gate |
...
| **V16** | **NEW 2026-09-21, from the zvec review (ideas only -- hard rule 6; Apache-2.0, no code).** **Cosine, as a storage decision rather than a refused metric.** `include/weave/vecscan.h:53` refuses cosine because a sound compressed-domain bound needs a maximum true norm that is not stored -- but zvec treats cosine as *normalize at ingest and keep the true norm per record*, and pg_weave **already stores a per-lane `(scale, norm)` pair** (V7). If the stored norm is the true one, cosine is L2 on normalized vectors and the existing bound transfers unchanged. F7 had to refuse `<=>` for exactly this reason, so this row is what unrefuses it. **Measure first (hard rule 9):** whether the stored `norm` is the pre-quantization true norm on every write path (build, insert, flush, merge) -- if any path stores a reconstruction's norm, the premise fails, and V7 already records that a reconstruction's norm is not its original's. | `<=>` becomes an ORDER BY member, its bound passes the (C2) property test, and recall against an exact cosine ranking is measured, not assumed |
| **V17** | **NEW 2026-09-21, from the zvec review.** **Switch plan strategy on predicate selectivity.** zvec materializes matching ids and scores them exactly below a selectivity ratio, and pushes a bitmap into the index above it. pg_weave has one strategy, and `VECTOR_CHANNEL.md` §9 already records that the scattered-filter case saves the fast kernels nothing -- which is precisely the shape a materialize-and-score-exactly plan wins. **This is claim 3's mechanism** (`ARCHITECTURE.md` §9: queries get faster as predicates get more selective), and claim 3 is currently **unmeasured** since F4's withdrawal. **Measure first:** the crossover, as a function of selectivity, between the gated fused scan and an exact rescore of the qualifying set. **PARTLY MEASURED 2026-09-23, and the measurement changes what this task should do** (`bench/RESULTS_GATE_SWEEP.md`, `bench/gatesweep.sh`): claim 3 now has numbers for the first time, and they are split. In CPU work the gated fused scan already behaves as claim 3 promises — pivots and vector `score()` calls are **selectivity to three digits** on three corpora, and scored code blocks follow `1 − (1 − s)^32` at nine of nine points. In page traffic it does not: **502 / 515 / 502 / 424** buffers at 100 % / 10 % / 1 % / 0.1 %, flat, because of `doc/GAPS.md` **G27**. So the *first* thing this task needs is not a second strategy to switch to — it is G27's block→page index, after which the existing strategy's page curve should follow its already-measured block curve. **G27 LANDED 2026-09-23 and it does: buffers at 0.1 % selectivity fell 1045 → 513 on scifact (2.04×) and 8727 → 1823 on fiqa (4.79×), with every work counter bit-identical.** So V17's crossover measurement can now be taken against a scan whose I/O actually responds to the predicate, which is the condition that would have made the earlier measurement meaningless. A materialize-and-rescore strategy is worth measuring against the *fixed* scan, not against one whose I/O is selectivity-independent by construction. Note also what the sweep exposes about the claim's wording: the only predicate pushable into the index today is another **lexical term**, because `WEAVE_CH_DOCVALS`'s page kind is reserved. | the crossover measured at two scales, the switch implemented at the measured point, and claim 3 supported by a number for the first time |
| **V18** | **NEW 2026-09-21, from the zvec review.** **Bit-plane progressive refinement** as an alternative axis to V15's two-stage prefix scan: score a coarse plane first and refine only survivors. It sidesteps V15's own blocker -- the `LANE` vs `VECMAJOR` pack conflict, since planes are sliced by bit significance rather than by coordinate -- and composes with the exact heap rerank (V10) instead of competing with it. **Measure first:** what fraction of lanes survive a 1-bit plane at the recall targets `bench/RESULTS_CODE_SCAN.md` already uses, on the same corpus, before any writer changes. Related zvec ideas recorded but not promoted: residual encoding, per-record affine quantization for calibration drift, strip **offset** addressing (already self-identified in §7.1 and blocking V10), batched M×N scoring, sparse vectors, group-by top-k. **And one useful negative: zvec has no late interaction at all** -- its "multi-vector" is multi-query RRF fusion -- so **V12 (ColBERT-style MaxSim) has no donor design** and stays a from-scratch task. | the survival fraction measured before anything is built; then a (C2)-sound plane bound |
...
| ~~**F2**~~ | **F2.1 AND F2.2 BOTH DONE 2026-09-21 FOR THE DOCID-SPACE CHANNELS; TWO CHANNEL FAMILIES ARE RECOGNIZED AND THEN REFUSED, WHICH IS WHY F8 AND F9 EXIST.** **F2.2:** `set_rel_pathlist_hook` in `src/am/fusepath.c` recognizes the shape F2.1's support function emits, maps each recovered `OpExpr` to an index column and channel, and builds an `IndexPath` whose `indexorderbys` are the per-channel operators plus the `<~>` **transport key** (strategy 5, `amstrategies` 4 -> 5) carrying the weights; `weave_rescan` builds one `lexshuttle` **per query term** plus required gate shuttles from the `WHERE` quals, and `weave_fuse_pass`/`_grow` runs the core per bolt with x4 geometric growth whose stop is a proof (no bolt's heap ever filled, so theta stayed -INF and nothing was pruned) rather than a ceiling. Extension **0.16.0**. **The refusals are the design:** `FUSED_TOPK.md` **section 7b** records that the channels do not share a position space -- lexical and gate shuttles publish docids, the vector shuttle publishes segment-local lane indices, `<@>` publishes dictionary positions -- so F2.2 decides the **docid space** and serves what already lives in it, and offers **no path at all** for the rest (G39 is what it looks like when an AM offers a path it then refuses). **A latent out-of-bounds write was found and fixed on the way:** `index_store_float8_orderby_distances()` loops over `numberOfOrderBys`, so the single-element array `weave_gettuple` passed would have been overrun the moment a scan had more than one order-by key (`amscan.c:2997`). **Owed:** the pushdown measured against the RRF control (`bench/RESULTS_FUSE.md`, section 8). **The original F2.1 half:** GATE MET (`sql/fuse_fallback.sql`, 269 tests green on 17.11 and 18.6 plus six lints); F2.2 OPEN.** Split after reading the code §7 describes — see `FUSED_TOPK.md` §7a, which corrects three of §7's claims and records one more falsification found while implementing: §7a (2)'s own `1/(1+S)` map is **superseded**, because it is total and monotone only for `S > −1` and a fused sum containing a recovered cosine *similarity* can cross that pole, which produces a **wrong order** rather than an error. `fuse()` returns `−Σ wᵢ·sᵢ` and ascending is best-first. **Landed (F2.1):** `src/am/fusepath.c` — the executable fallback (`fuse()`, one C symbol for the 2..8-argument overloads via `PG_NARGS()`, summed left-to-right as a stated contract because the fused scan sums in a different order and the two are therefore not bit-identical), the two score-recovery functions (`weave_lexscore` = `1/d − 1`, `weave_cosscore` = `1 − d`, both total on every float because raising inside a sort key turns a user's arithmetic slip into a failed query), and the **planner support function**, which on `SupportRequestSimplify` rewrites each argument whose operator names a channel into a recovery call **wrapping the original `OpExpr` intact**, so F2.2's matcher has the operator in front of it. Idempotent by construction (only an `OpExpr` is rewritten, and it returns request-unhandled unless something changed). Operator OIDs are resolved through the **extension's own schema**, never `search_path`, cached keyed on the extension OID, and a failed lookup **declines to rewrite** rather than erroring, because this runs in the planner for any query mentioning `fuse()`. Weights: one per score, finite and strictly positive, validated by one shared routine so the plan-time and per-row paths cannot disagree; `NULL` weights means equal weights of **1.0, unnormalized**, so adding a channel does not rescale the others. **Plan shape decided by the maintainer 2026-09-21: (A) a hand-built `IndexPath` in `set_rel_pathlist_hook`** with the individual `col <=> q` `OpExpr`s plus one **transport key** (`<~>` over `(wdoc, float4[])` and `(wvec, float4[])`) carrying the weights; a `CustomScan` was rejected (it re-implements `nodeIndexscan`'s heap fetch, visibility, qual recheck and EPQ) and a *qual* transport key was rejected (`indexqualorig` is re-evaluated on an EPQ recheck, so the marker would eventually execute). **None of that pushdown code is written yet, deliberately:** a path nothing calls is how V8's contract grew five defects, so it lands with the AM side in F2.2. **Still owed:** F2.2 (the pushdown and the AM-side fused scan) and `sql/fuse_degenerate.sql`, inherited from F1, which only becomes meaningful once F6/F7 give it a second scored channel. Extension **0.14.0**; the eleven expected-output changes are the `ALTER EXTENSION UPDATE` notice and were proved to be that line and nothing else before regeneration (hard rule 3). | `sql/fuse_fallback.sql` — **MET**: the rewrite asserted from `EXPLAIN` (plain, commutator, and partially-unrecognized spellings), the value checked per row against the same formula written by hand, five rejections, the 8-channel overload, and the §7a (1) divergence **recorded rather than asserted away** — the index's exact-BM25 ranking and the same query with no index path differ in all 5 positions, with an `EXPLAIN` over each arm proving each ran the plan it claims |
...
| ~~**F5**~~ | **DONE 2026-09-20. GATE MET: 1,140,000 trials, 37,765,994 checks, 0 failures in 82 s** (dev host c7i.2xlarge, `gcc -O2 -Wall -Wextra -Werror`, also wired into `make check-standalone` at 60k trials). `test/hegel/test_fuse_props.c` compares the core against `weave_fuse_reference()` — a brute-force oracle in the same TU that uses only `seek()` and `score()` and **never asks for a bound**, so a (C2) violation cannot move it. **Eight properties at the F5 gate, nine since 2026-09-23**: fused == brute force including the tie-break; **no backward seek**, asserted by the synthetic channel itself; `score()` only at `cur`; `score()` calls ≤ the reference's; no tombstoned row returned; **gates applied conjunctively**; the (C2) check and every init refusal; and **P8, a positive control that answers "can this test see the failure mode hard rule 1 is about"** — with the check off and one channel's bound scaled below the truth, **0.99 changes the answer in 1.30 % of trials and 0.90 in 13.08 %**, and the run FAILS if either rate is zero. That 1.3 % is the finding, not the pass: it is the measurement of *how silent* a 1 %-too-low bound is, and why no fixed-expected-output test can catch one. **P9, added 2026-09-23, is the positive control the G43 commit owed:** the *abandonment* audit (`WEAVE_FUSE_C2_ABANDON`) had never fired on anything, and a hand-built two-channel fixture makes it fire — the generator cannot, because the audit needs four conditions at once (finite theta, the sabotaged channel sorting last, the leading channel's block max above its score at the pivot, a true contribution big enough to cross theta). Its third leg runs the same sabotage with the check **off** and gets no error, a plausible answer and the wrong document; a mutant that disables the abandonment *prune* makes the sabotage surface as `C2_VIOLATION` instead, so which check fires is decided by a prune rather than by the bound. **Coverage is asserted, not hoped for** — the run fails if any prune path stayed at zero, and at 10^6 trials it reports pivot 79.4 M, block skips 32.2 M, required-intersection skips 20.0 M, vetoes 4.8 M, abandonments 10.0 M, tombstone drops 14.7 M, and 471,794 trials that demoted a channel to non-essential. **Mutation: 11 of 13 legs killed** (`/scratch/pg_weave/f1-mut.sh`, with a baseline leg that must pass first and a per-leg timeout, both added after a killed ssh session left a mutation in the working copy and cost an hour). The two survivors are declared with mechanisms: `suffix[0] <= θ + 0.001f` is unsound but reachable only when the k-th best score sits within 0.001 of the sum of every channel's ceiling (the `+ 1.0f` leg IS killed, so the check is exercised), and `s > θ` → `s >= θ` at the heap push is unreachable because the abandonment test already discards a score equal to θ and the global-termination check breaks a pure-boolean scan first — so that comparison is defensive redundancy, and the load-bearing tie-break is `fuse_worse()` plus those two earlier discards. **One mutant HUNG rather than failed**, which found a real defect: `rend < p` sent the scan backwards and it cycled forever, so the invariant is now refused by name instead of assumed. | ~~passes 10^6 generated cases~~ **MET** |
| ~~**F6**~~ | **DONE 2026-09-21, GATE MET: 11,715,844 checks, 0 failures, coverage asserted** (near-tight 433,348, attained-exactly 200,000, bound > 2x 237,787). Created the same day by F2 reading the code: the BM25 lexical channel had no shuttle, because ranked lexical is a Broder/BMW WAND over `WandCursor` and the fused core consumes only `WeaveShuttle`s. **Landed:** `include/weave/bm25bound.h` (the backend-free core — `weave_bm25_factors_init`, `weave_bm25_contrib`, `weave_bm25_block_bound`, `weave_bm25_term_bound`), `src/query/lexshuttle.c` (the `WeaveShuttleOps` skin over one query TERM, no caller until F2.2 — the `gate.c` arrangement), `test/hegel/test_lexbound.c` in `make check-standalone`. **The point of the task is that `src/am/amscan.c` now CALLS the header rather than keeping a second transcription of the formula** — a duplicated bound is a property test that tests the copy nobody runs. **Two deviations from bit-exactness, both reported rather than hidden.** (1) The block bound's numerator was regrouped from `idf*max_tf*(k1+1)` to `idf_k1p1*max_tf` (≤ 1 ULP) because otherwise the bound is **not** attainable at the extreme: two roundings of one real product are not ordered, so the old expression could sit one ULP *below* `score()`, which is a real (C2) violation rather than a tidiness issue. Answer-preserving, and the argument is checkable: the prune fires only when `nheap >= k`, every document it drops scores ≤ blocksum ≤ threshold, and admission is strictly `score > threshold` (`amscan.c:5198`), so a 1-ULP bound change cannot move heap membership, order, or any printed score — and no expected output moved. (2) `factors_init` clamps `avgdl <= 0` to 1.0 exactly as `rank.c:130` already does, unreachable from a scan (a cursor exists only if a posting exists) and present so the header is total. The v4 quantized-doclen floor stays in `amscan.c`: it is a sidecar property, not BM25. Positive control: scaling the bound to 0.999 produces 9,902 failures. | **MET:** (C2) `block_bound >= contribution` over randomized `(idf, k1, b, avgdl, max_tf, min_doclen)` with `tf <= max_tf` and `doclen >= min_doclen`; bitwise equality at the attaining extreme; monotonicity in both variables; finiteness including `avgdl = 0`, `b = 0`, `b = 1`, `k1 = 0`; `maxscore >= block_max`; params-form == factors-form. 200,000 trials by default, `argv[1]` override |
| ~~**F7**~~ | **DONE 2026-09-21, GATE MET (`sql/vecorderby.sql`, green on 17.11 and 18.6), AND THE FIRST CUT OF IT WAS A WRONG ANSWER THAT THE CORRECTED TEST QUANTIFIES.** Created the same day by F2 reading the code: `wvec_weave_ops` was `STORAGE wvec` and nothing else, so a vector query could not reach `amrescan` at all and `ORDER BY embedding <=> $1 LIMIT 10` — the first query shape any pgvector user writes — was a Seq Scan plus a top-N Sort. **The mistake, kept on the record because the number is the lesson:** the first cut registered **`<=>`**, which is *cosine* distance, while `include/weave/vecscan.h:53` says the scan core serves `WEAVE_METRIC_IP` and `WEAVE_METRIC_L2` and **refuses cosine**, and the weft's metric is the `metric` reloption (default l2). So the index answered an **l2 ordering under a cosine operator** — a wrong answer, not an approximation — and the file's own "quantizer divergence" section was measuring it: **24 of 25 positions differing, overlap 19 of 25.** After registering the operators whose semantics the weft actually computes, the same section measures **4 of 25 positions and overlap 25 of 25**, which is the quantizer alone. An ORDER BY operator an index serves must be the ordering the index computes. **Landed:** `<->` (strategy **1**, `WEAVE_STRAT_VEC_L2`, ascending = nearest) and `<#>` (strategy **4**, `WEAVE_STRAT_VEC_IP`, pgvector's negative inner product, ascending = largest inner product) as ORDER BY members of `wvec_weave_ops`; `amstrategies` **3 → 4**, since `ALTER OPERATOR FAMILY` validates the member number against it and order-by keys are dispatched on `sk_strategy` alone; a metric mismatch is **refused** in `weave_rescan` with an error naming both the operator and the index's metric plus a `HINT`; `<=>` is deliberately **not** a member and a test asserts it now plans a Sort over a Seq Scan, which is the honest plan. The SRF and the ordering scan were made to **share one scoring loop** (`weave_vec_topk_run`, `include/weave/vector.h`), so the equality test cannot detect a scoring bug and instead pins what F7 added: docid→TID resolution, the visibility probe, the widening ladder, and the order those preserve. Extension **0.15.0**. **Still owed, and it is Phase V's debt:** one operator family per metric (`wvec_l2_ops`, `wvec_ip_ops`), `VECTOR_CHANNEL.md` §8b — the metric belongs in the opclass the way pgvector does it, not in a reloption the planner cannot see and that `ALTER INDEX ... SET` can drift away from `WeaveVecMeta.metric` without a REINDEX. **One PG18-only hazard found on the way, pre-existing and now G39:** with `enable_seqscan = off`, PostgreSQL 18 compares disabled-node counts *before* costs, so it picks the query-less Index Only Scan that `weave_costestimate` prices at 1e12 and `weave_gettuple` refuses — a bare `count(*)` on an indexed table then fails. | **MET:** `EXPLAIN` shows `Index Scan` + `Order By` (and the `LIMIT`-less form); rows equal `weave_vec_scan()` at k = 1, 5, 100, 1000 and limitless; a narrow answer is the prefix of a wide one (the widening extends rather than restarts); two bolts merge into one ordering and the second bolt is reachable; G29's pending-buffer asymmetry recorded as the behaviour it is; HOT-updated and deleted rows resolve correctly; correlated-subquery and LATERAL rescans; both metric refusals; and the quantizer divergence recorded with an `EXPLAIN` over each arm |
| ~~**F8**~~ | **DONE 2026-09-22. The flagship `fuse(body <=> q, emb <-> v)` query is answered by the index.** `include/weave/vecdocmap.h` + `src/am/vecdocmap.c` relabel a lane-space `WeaveFuseChan` into the docid space through the bolt's docid-ascending warp map; the AM side (`weave_fuse_rescan`/`weave_fuse_pass`) accepts `<->` and `<#>` order-by keys, materializes the map once per bolt per pass, and builds the shuttle behind an adapter. Extension **0.17.0**; `amstrategies` unchanged (F7 had already registered both members). **FOUR defects fixed, and only two of them were in the adapter** -- `FUSED_TOPK.md` **section 7c** carries all four with their mechanisms: (1) **`doc/GAPS.md` G41**, a live wrong answer -- `fuse()` had no score-recovery function for `<->`, `<#>` or `<@>`, so the raw DISTANCE entered the sum as a score and the final negation ranked the FARTHEST vector first, in a query shape this spec has advertised since it was written, printed into `expected/fuse_pushdown.out` and reviewed three times; (2) the metric mismatch is now a **PLAN-TIME** refusal (`src/am/fusepath.c` opens the index and reads the reloption through a new NON-THROWING accessor `weave_index_vec_metric_raw()`), where the single-channel `ORDER BY` path can still only raise at rescan -- which narrows what `VECTOR_CHANNEL.md` section 8b's per-metric opclass split is still needed for; (3) duplicate docids are **refused**, because the first version's argument for accepting them conflated the docid interval with the lane block the inner bound describes, and was wrong in both (C2) and (C4); (4) `WEAVE_FUSE_END` has to be **latched**, or an exhausted inner channel gets seeked backwards and a real shuttle RAISES -- an ERROR mid-scan on a good index rather than a wrong answer. Plus (5) the vector channel now filters **its own** segment's tombstones, which section 7b's (C6) note requires of every channel and which the single-channel path leaves to the heap and its widening ladder; the probe rides the warp-map pass, so it is free where it lands. | **MET.** `test/hegel/test_vecdocmap.c`: **18,899,792 checks, 0 failures**, nine properties, two positive controls firing at 32.4 % and 20.3 % (an unclamped `blkend`, and a flipped `lower_bound` comparator). P9 drives the REAL `weave_fuse_init`/`run`/`drain` over a mixed adapted + native channel set against `weave_fuse_reference`. `sql/fuse_pushdown.sql` section 2b: the flagship query plans an `Index Scan` with three `Order By` keys, its rows **equal** the exact oracle built from `weave_search()` plus `weave_vec_scan()`, the oracle is asserted tie-free first, and the ordering DIRECTION is reported on its own -- because an oracle agreement could still hide G41 if the oracle were built the same wrong way |
| **F9** | **NEW 2026-09-21, created by F2.2; HALF OF ITS PREREQUISITE SHIPPED IN F8.** `weave_edistscore(d) = -d` landed in 0.17.0 (`doc/GAPS.md` G41), so the FALLBACK's arithmetic for `fuse(..., body <@> p)` is correct now -- it was ranking the worst spelling match first. What F9 still owes is the pushdown. Give `<@>` (Z9 edit distance) a **document-space** shuttle. Unlike F8 this is not an adapter: `src/query/edist.c` walks the **dictionary**, so its positions are dictionary entries and there is no relabelling that turns them into docids -- the channel has to publish document positions, which means merging the postings of the terms it accepts behind the cursor. That is the same fanout the Z5/Z6 rows hit, so this task is where a bounded `<@>` either gets cheap or gets recorded as a loss. | `<@>` participates in a fused pushdown; (C1)+(C2) property test per hard rule 1; and the fanout cost measured rather than assumed (hard rule 9) |
...
task id is invented for it:~~ (a) was TAKEN 2026-09-22 as a MEASUREMENT decision; (c) was
TAKEN 2026-09-23 and WITHDRAWN THE SAME DAY BY MEASUREMENT
...
**DECISION (c) TAKEN AND WITHDRAWN, 2026-09-23, and the withdrawal cost an hour instead of a
format change.** The maintainer chose (c) — cluster-order the weft and reopen F8. Before any
code: `bench/code_scan.c` already carries `order=clustered|natural` with a real k-means, so
the only new code needed was a TSV→`.fvecs` converter (`bench/tsv2fvecs.py`), and the
question was answered on the three BEIR corpora the nDCG row itself uses. **0.00 % of blocks
pruned and 100.00 % of lanes scored in BOTH orderings, on all three, and 0.00 % against an
ORACLE threshold** — the ceiling over every possible block ordering. The clustered arm's
blocks are genuinely tighter (mean radius 1.0436 → 0.9992 on scifact, and B3 falls with it),
and it changes nothing, because the bound needs a **2.12–3.40× smaller radius**, not a 4 %
smaller one. Sweeping the cluster count makes it *worse*: at `lists > n/32` one 32-lane block
spans several clusters, so its bounding radius is the union's and returns to the
natural-order value. The format version bump, the 4-byte/doc docid→lane indirection, the
merge-sort-to-re-clustering rewrite and the reopening of F8 would all have bought zero.
**Hard rule 9 paying for itself for the second time on this exact bound.** Only option (b) is
left, and it attacks bytes-per-lane rather than lanes-touched, so this result does not bear
on it. See `bench/RESULTS_CLUSTER_ORDER.md` for the table, the radius inversion and the
`lists` sweep.
...
| ~~`score()` calls~~ | ≤ 0.20× | 0.571× | 0.875× | 0.513× | 0.648× / 0.903× / 0.541× | **FAIL — and the row is SUPERSEDED 2026-09-22 by the per-channel restatement below; the unit changed, the verdict did not** |
...
either.~~ BOTH DECIDED 2026-09-22 — the presentation stays as written because it is what the
decisions were made from, and no task id is invented for either.** (1) **Should
`pg_weave.fuse_normalize` stay ON by default?** On: the ranking beats RRF
...
**DECIDED 2026-09-22 — both of them, and the gate does not move: it is still 2 of 5.**

  1. **`pg_weave.fuse_normalize` STAYS ON BY DEFAULT.** A user chooses a **ranking**, not a
     scan strategy: off, the fused objective loses to a plain RRF control on all three corpora
     measured (0.982× / 0.924× / 0.687×), and a fused scan that ranks worse than the two-query
     control it replaces has no reason to exist; on, it beats RRF everywhere measured (1.053× /
     1.010× / 1.114×). **The price is recorded, not hidden:** p99 **PASS → FAIL** (0.609× /
     0.560× / 0.633× → 0.710× / 0.612× / 1.000×), p50 0.582× / 0.795× / 0.578× → 0.710× /
     0.827× / 1.172×, fiqa slower than the control. `PGC_USERSET` means a deployment can have
     the old trade per query or per session, **which is why this is a default and not a fork in
     the design**, and the latency regression is **not accepted as permanent** — it is charged
     to the single blocker below.
  2. **§8's work row is RESTATED per channel**, each channel in its own unit: lexical **BM25
     contributions** (gate ≤ 0.20×), vector **CODE BLOCKS READ** against the control's own code
     scan (gate ≤ 0.20×), and **pivots per query reported but NOT gated**, because the RRF
     control has no pivot loop and there is nothing to take a ratio against. Forced by two
     measurements: a one-lane read in `WEAVE_PACK_LANE` touches every byte of its block, so a
     **lane is not a unit of cost and a block is** (measured in the V15/V16 work,
     `bench/RESULTS_CODE_SCAN.md:330,417`); and the lane-based row said normalization made the
     fused arm *cheaper* on all three datasets while the clock said fiqa's p50 **doubled**,
     which is a work row that cannot predict the row it stands in for. The pivot count is what
     tracks the clock (fiqa pivots 5.3×, p50 2.0×). **The restatement does NOT rescue the row**
     — in blocks the vector ratio is **1.000×**, exactly as in lanes — so it buys honesty about
     the unit, not a pass. This is §8d's option (a), taken as a **measurement** decision only.

**The work row in the new units** (local, deterministic, `bench/normprod.sh` with
`PHASES=work`; the harness computes the ratios itself in both `bench/normprod.sh` and
`bench/fuse.sh` and the two agree to the digit on nfcorpus):

| row | gate | scifact | nfcorpus | fiqa | |
|---|---|---|---|---|---|
| lexical work: BM25 contributions | ≤ 0.20× | 0.203× | 0.380× | **0.052×** | **MET on fiqa**, missed on the other two (scifact just over) |
...
**The abandonment fact, because it explains both halves at once: incremental abandonment fires
constantly, not rarely** — on fiqa **36,709,890 abandonments over 37,306,460 pivots, 0.98 per
pivot** — and that is what produced the lexical improvement (2.8× fewer BM25 contributions). It
cannot help the vector channel for an arithmetic reason: the fused core sorts scored channels by
**descending weighted ceiling** (`src/am/fuse.c:163-165`) and abandons on
`s + csuffix[j+1] <= theta` (`:716`), so the vector channel — weight **0.4947** against
**0.0110** per lexical channel after normalization — is summed **first** and scored before any
abandonment test runs. **Reversing the order is DEAD ON ARITHMETIC, not on effort:** the test
would be `s_lex + w_vec * block_max_vec <= theta`, and `w_vec * block_max` is ~0.49 against a
theta of ~0.11, so it can never fire; it would still be dead at k = 10 rather than the ladder's
k = 128. Recorded so nobody spends a week on it. And a per-block vector bound carries no
information on L2-normalized data ((B1), (B2), (B3) all ≈ 1.0 unless a block is coherent in
direction), so **the single blocker is the size of the vector channel's candidate set**, and it
runs through (b) or (c) above. `bench/RESULTS_FUSE.md` (fifth measurement), `doc/GAPS.md`
...
is a positive control for the new abandonment audit, which has never fired.~~ **PAID
2026-09-23: P9 in `test/hegel/test_fuse_props.c`.** A hand-built two-channel fixture (the
random generator cannot reach the audit — it needs a finite theta, the sabotaged channel
sorting last, the leading channel's block max above its score at the pivot, and a true
contribution big enough to carry the document back over theta, all at once), three legs, two
mutants. Its third leg is hard rule 1 in one fixture: bound 100× too low with the check
**off** returns no error, a plausible answer, and the wrong document. And the mutant that
disables the abandonment *prune* makes the same sabotage surface as `C2_VIOLATION` instead —
so which of the two checks fires is decided by a prune, not by the bound, which is why P7
never covered this. `doc/GAPS.md` G43.
