...
| **V15** | **RE-PROMOTED 2026-09-15, after being demoted the same day; read the sequence in `bench/RESULTS_CODE_SCAN.md` before touching this.** **Two-stage prefix scan.** Stage 1 scores every vector against the first `m` of `dim` coordinates and keeps the best `W`; stage 2 rescores those `W` over all coordinates; stage 3 is V10's exact heap rerank of the top 25. **The verdict moved three times and every move was downstream of one harness error**, which is the part worth remembering: (a) before V16's kernel existed V15 was credited with making the latency gate pass at 1.51x; (b) with the fast kernel it measured 64.6 ms against a 60.7 ms flat scan at equal recall and was demoted to a knob; (c) the harness's stage 2 was scoring **one whole 32-lane block per survivor** (`livemask = 1 << sl`), which wastes 31/32 of every call under `lut-byte` because that kernel scores all 32 lanes regardless of mask. Grouped by block, stage 2 falls ~20.1 -> 13.5 ms and **prefix 0.5 / W 8000 is 52.8 ms at recall 1.0000 against 75.9 ms for the full-dim arm at the same recall -- 1.44x, for free, and 0.72x pgvector HNSW at matched recall.** That is the configuration to build. **Two design facts the measurement exposed, both load-bearing:** grouping must not change the order results are pushed in -- a bounded top-k rejects on `s <= theta`, so equal scores are broken by insertion order and reordering stage 2 moved recall 1.5 points at n=200k; and there is **no per-vector shortcut in `WEAVE_PACK_LANE`**, because coordinate `j` of lane `s` is one nibble at byte `j*16 + s/2`, so reading one lane touches every byte of the block and scoring 1 lane costs the same memory traffic as scoring 32. The layout in which stage 2 would be cheap is `WEAVE_PACK_VECMAJOR`; stage 1 wants LANE and stage 2 wants VECMAJOR, and no index has both without storing its codes twice, which forfeits the storage gate. It needs no new on-disk structure: the rotation makes coordinates exchangeable so a prefix is an unbiased subsample, and `WEAVE_PACK_LANE`'s code index is `j * 32 + slot`, independent of dim, so a coordinate prefix is a contiguous prefix of the block; it is layout-specific and must assert `WEAVE_PACK_LANE`. `m` and `W` are query-time GUCs (they change no stored bytes): `pg_weave.vec_prefix_frac` and `pg_weave.vec_prefix_window`. `W` scales **sublinearly** in n -- 8000 at n=100k matches 20000 at n=1M -- so the default must be documented as a floor that grows, the way V10's rerank window is. | stage 2 grouped by block, with a test that grouping changes **no** score and **no** returned row -- the insertion-order trap above makes that a correctness gate, not a tidiness one; end-to-end recall@10 >= 0.99 at n >= 1M **with nq >= 100** (every n=1M recall figure on record is nq=10, i.e. 100 GT slots, which cannot separate 0.99 from 1.00) on two corpora of different dimensionality, measured against **exact search over the same table**; p50 <= 2x pgvector HNSW at matched recall, warm and cold, on the same instance type; a property test that stage 1 + stage 2 returns exactly the flat scan's top-k whenever W >= n; and a hard assertion that the pack layout is `WEAVE_PACK_LANE` |
...
**The prefix scan (V15) then came back**, once the harness stopped overcharging its
stage 2 by scoring a whole 32-lane block per survivor. Grouped by block, **prefix 0.5 /
W 8000 is 52.8 ms at recall 1.0000 -- 1.44x the full-dim arm at equal recall, and 0.72x
pgvector HNSW.** So the term is met by the two levers together, and the honest summary of
this phase's last two days is that **the kernel was the larger factor and the prefix scan
is still worth having, and it took three measurements to say that** because the first two
were taken through a harness artifact. See V15's row and
`bench/RESULTS_CODE_SCAN.md`.
