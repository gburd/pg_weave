...
| Z6 | Route regex (`/re/`) through regex AST → trigram tiling → vocabulary candidates. **Before trusting a tile for pruning, read `src/query/uleven.c`'s header:** a `k≥2` tile always falls back to `always_true` at the `max_out` `tiling.c` passes, so any measurement that appears to show `k≥2` tile pruning is measuring something else | character-class regex `E-[0-9]{4}` p50 ≤ 100 ms (pg_tre measured 1.5 s) |
