...
| **L15** | **Cut the build's hash cost.** `bench/RESULTS_BUILD_PROFILE.md` attributes **37.5% of build time** to `hash_search_with_hash_value` — 17.4% in the merge, 13.6% in `add_posting` under the build scan. The build's `TermKey` is a fixed 64-byte blob under `HASH_BLOBS`, so every one of ~240M term-occurrence lookups hashes and `memcmp`s 64 bytes where a typical term needs ~10. Step 1: store the length and supply `HASH_FUNCTION`/`HASH_COMPARE` that respect it (~20 lines, cuts both the hash input and the compare). Step 2, only if step 1's measurement justifies it: replace dynahash with `lib/simplehash.h`, whose open addressing avoids the bucket-chain pointer chasing. | `bench/RESULTS_BUILD_PROFILE.md` | build ≤ 260 s on `synth-2m-long` with index size unchanged at 625 MB and no ranked-latency regression |
