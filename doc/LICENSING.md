...
| Sparsemap (succinct bitmap, used for tombstones and trigram postings) | `src/util/sparsemap.c`, `include/weave/sparsemap_impl.h` | vendored at **v5.5.1** (`SM_VERSION_STRING`), upstream `83e7b01`. Namespaced fork, so a bump is a manual merge: apart from the include block, `sparsemap.c` is byte-identical to upstream `sm.c` and `sparsemap_impl.h` to upstream `sm.h` — diff against the tag to keep the next bump mechanical. The 5.5.0->5.5.1 delta is `__sm_append_data()` returning `bool` under `warn_unused_result`, checked at every call site; it is NOT implicated in the merge tombstone P0 | **MIT**, © 2024 Gregory Burd | vendored, symbols namespaced |
