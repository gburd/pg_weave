...
# other backends while it runs.  pg_weave routes every aminsert to the pending
...
setup
{
    ...
    CREATE TABLE cic (id int primary key, d wdoc);
    INSERT INTO cic SELECT g, to_wdoc('alpha doc ' || g)
...
session "builder"
step "b_cic"   { CREATE INDEX CONCURRENTLY cic_bm25 ON cic USING weave (d); }
step "b_ric"   { REINDEX INDEX CONCURRENTLY cic_bm25; }
step "b_check" { SET enable_seqscan = off;
                 SELECT count(*) AS alpha FROM cic WHERE d @@@ 'alpha'::wquery;
                 SELECT count(*) AS beta  FROM cic WHERE d @@@ 'beta'::wquery; }

session "writer"
step "w_beta"  { INSERT INTO cic SELECT g, to_wdoc('beta doc ' || g)
