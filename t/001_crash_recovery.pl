...
# Crash-recovery / WAL-replay correctness for the pg_weave weave index.
#
# Every page mutation in pg_weave goes through GenericXLog, so an immediate
...
my $rank_before = $node->safe_psql('postgres',
	"$q SELECT string_agg(id::text, ',') FROM (SELECT id FROM docs WHERE d \@\@\@ 'alpha'::wquery ORDER BY d <=> 'alpha'::wquery LIMIT 10) x"
);
...
is($fc_after,    $fc_before,    'weave_count survives crash recovery');
...
$node->safe_psql('postgres',
	"INSERT INTO docs VALUES (99999, to_wdoc('alpha extra'))");
my $alpha_grown =
  $node->safe_psql('postgres',
	"$q SELECT count(*) FROM docs WHERE d \@\@\@ 'alpha'::wquery");
