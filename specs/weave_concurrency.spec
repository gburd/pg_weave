...
#   * a concurrent VACUUM / weave_merge does not change what an open snapshot's
...
setup
{
    ...
    CREATE TABLE tdoc (id int primary key, d wdoc);
    INSERT INTO tdoc SELECT g, to_wdoc('alpha doc ' || g)
        FROM generate_series(1, 200) g;
    CREATE INDEX tdoc_weave ON tdoc USING weave (d);
}
...
step "r_alpha"  { SELECT count(*) AS alpha FROM tdoc WHERE d @@@ 'alpha'::wquery; }
step "r_beta"   { SELECT count(*) AS beta  FROM tdoc WHERE d @@@ 'beta'::wquery; }
step "r_commit" { COMMIT; }

# Fresh READ COMMITTED reader (new snapshot per statement, auto-commit).
session "f"
setup           { SET enable_seqscan = off; }
step "f_beta"   { SELECT count(*) AS beta  FROM tdoc WHERE d @@@ 'beta'::wquery; }
step "f_alpha"  { SELECT count(*) AS alpha FROM tdoc WHERE d @@@ 'alpha'::wquery; }
step "f_gamma"  { SELECT count(*) AS gamma FROM tdoc WHERE d @@@ 'gamma'::wquery; }

# Writer.
session "w"
step "w_beta"   { INSERT INTO tdoc SELECT g, to_wdoc('beta doc ' || g)
...
step "w_merge"  { SELECT weave_merge('tdoc_weave'); }
step "w_reuse"  { INSERT INTO tdoc SELECT g, to_wdoc('gamma doc ' || g)
